        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前的路由跳转：从路由A跳转到路由B，
                这里加入购物车，进行路由跳转之前，发请求把你购买的产品信息通过请求的形式通知服务器，服务器进行相应的存储 -->
  ...
  export default {
    ...
    methods:{
      ...
      changeActive(saleAttrValue,arr){
        // 遍历全部售卖属性值isChecked为零，全部取消高亮（排他思想）
        arr.forEach(item => {
          item.isChecked = '0';
        });
        // 点击的属性值需要高亮
      ...
      async addShopCart(){
        ...
        try {
          // 成功
          await this.$store.dispatch('addOrUpdtateShopCart',{skuId:this.$route.params.skuid,skuNum:this.skuNum});
          // 路由跳转
          // 跳转时还需要将产品的信息带给下一级路由组件
          // 可以用下面这行代码传递参数，但地址栏显示的参数很恶心
          // this.$router.push({name:'addcartsuccess',query:{skuInfo:this.skuInfo,skuNum:this.skuNum}})
          // 简单数据通过query形式给路由组件传递过去
          // 复杂数据通过会话存储session storage （不会持久化，会话结束就消失）
          // 本地存储和会话存储  一般存储的是字符串
